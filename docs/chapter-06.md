...
클로드가 Supabase를 연결할 때 **API Key**라는 단어가 나옵니다.

**API Key = 로그인 비밀번호 같은 것**

우리 앱이 Supabase 데이터베이스에 접속하려면 "나 인증된 사용자야"라고 증명해야 합니다.
그 증명서가 바로 API Key입니다.

> **중요: API Key는 절대 공개하면 안 됩니다!**
> - 남이 내 API Key를 쓰면 **내 계정에 요금이 청구**됩니다
> - 서버가 털리면 API Key도 유출될 수 있습니다
> - 비밀번호처럼 소중하게 관리하세요

공인인증서나 간편인증처럼 귀찮아도 매번 해야 하는 절차입니다.
다른 곳에 있는 서비스(Supabase)를 연결할 때는 이런 인증 과정이 필요합니다.
